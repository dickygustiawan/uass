//@ts-check
import {not} from '../../../lib/swr-fetcer'
import Link from 'next/link'
import {mutate} from 'swr';
import Router from 'next/router';
export default function datnot(){
    const {data, error}= not();
    if(error){
        return <div>
            Error loading
        </div>
            ...
            {data.map((usr, idx)=>(
                <tr key ={idx}>
                    ...
                    <div className='d-flex justify-content-between'>
                        <Link href={`/admin/nota/updatenota?id_nota=${usr.id_nota}&id_user=${usr.id_user}&id_barang=${usr.id_barang}&qty=${usr.qty}&total_harga=${usr.total_harga}`}>
                            <a>Edit</a>
                        </Link>
